...
class KorisniciKontroler extends Controller {
	...
	public function postUcitaj(){
			...
			return json_encode(Skole::rightjoin('korisnici','korisnici.id','=','skole.korisnici_id')
				->where('korisnici.prava_pristupa_id',4)
					...
					$query->where('korisnici.prezime','Like','%'.(isset($_POST['pretraga'])?$_POST['pretraga']:'').'%')
						...
						->orWhere('korisnici.email','Like','%'.(isset($_POST['pretraga'])?$_POST['pretraga']:'').'%')
						->orWhere('korisnici.telefon','Like','%'.(isset($_POST['pretraga'])?$_POST['pretraga']:'').'%');
					})
					->get(['korisnici.id','prezime','ime','korisnici.email','korisnici.telefon','korisnici.aktivan','skole.naziv','skole.sediste'])
					->toArray());

			/*switch(Session::get('prava_pristupa')){case 5:
